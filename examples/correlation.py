...
def correlation_clustering(similarity):
    ...
    for j in xrange(items):
        for i in xrange(j):
            ...
            lp.cols[index].upperbound = 1     # Each variable in range 0 to 1.
    ...
    for row in lp.rows:                      # For each row.
        row.upperbound = 1                   # Each row is <= 1.
    lp.solve()                               # Run the optimization.
    labels = [[lp.cols[pair2index(i,j)].solution for i in xrange(j)]
              for j in xrange(items)]
